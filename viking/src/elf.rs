...
pub fn load_orig_elf(version: &Option<String>) -> Result<OwnedElf> {
    ...
    let data_name = version.as_ref().map(|s| format!("data/{}", s)).unwrap_or("data".to_string());
    path.push(data_name);
    path.push("main.elf");
...
pub fn load_decomp_elf(version: &Option<String>) -> Result<OwnedElf> {
    let build_dir_name = version.as_ref().map(|s| format!("build/{}", s)).unwrap_or("build".to_string());
