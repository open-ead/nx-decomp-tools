...
fn parse_args() -> Result<Args, lexopt::Error> {
    ...
    let mut version = repo::CONFIG.get("default_version").map(|s| s.as_str()).unwrap_or(None).map(|s| s.to_string());
...
fn main() -> Result<()> {
    ...
    if let Some(func) = args.function {
        ...
        // Normal check mode.
        check_all(&functions, &checker, &orig_elf, &decomp_elf, &decomp_symtab, args.update_matching, &version)?;
    }
