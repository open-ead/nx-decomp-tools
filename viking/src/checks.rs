    ...
    FunctionChecker<'a, 'functions, 'orig_elf, 'decomp_elf>
{
    pub fn new(
        ...
        let orig_got_section = if repo::get_config().skip_got_section { None } else { Some(elf::find_section(orig_elf, ".got")?) };
    ...
    fn check_data_symbol_ptr(
        ...
        if !repo::get_config().skip_got_section && !elf::is_in_section(self.orig_got_section?, orig_addr_ptr, 8) {
            return None;
        }
